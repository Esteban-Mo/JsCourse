...
export const chapter: Chapter = {
  ...
  quiz: [
    ...
    {
      question: 'Quelle API observe les modifications du DOM en temps réel (ajout/suppression de nœuds, changements d\'attributs) ?',
      sub: 'Observer les changements du DOM.',
      options: ['ResizeObserver', 'IntersectionObserver', 'MutationObserver', 'PerformanceObserver'],
      correct: 2,
      explanation: '✅ Exact ! MutationObserver surveille les modifications structurelles du DOM : ajout ou suppression de nœuds enfants (childList: true), changements d\'attributs (attributes: true), et modifications de texte (characterData: true). On peut aussi surveiller tous les descendants avec subtree: true. Il remplace l\'ancienne API DOMNodeInserted qui était synchrone et catastrophique pour les performances.',
    },
    {
      question: 'Que contrôle l\'option threshold dans IntersectionObserver ?',
      sub: 'Configuration de l\'IntersectionObserver.',
      options: [
        'La marge autour du viewport pour la détection anticipée',
        'Le délai minimum en millisecondes entre deux callbacks',
        'Le pourcentage de l\'élément visible qui déclenche le callback',
        'Le nombre maximum d\'éléments observés simultanément',
      ],
      correct: 2,
      explanation: '✅ Exact ! threshold définit à quel pourcentage de visibilité le callback est déclenché. 0 = dès qu\'un pixel est visible, 1 = seulement quand l\'élément est entièrement visible, [0, 0.5, 1] = callback déclenché à trois moments distincts. C\'est à ne pas confondre avec rootMargin, qui ajoute une marge invisible autour du viewport pour déclencher la détection avant que l\'élément soit réellement visible.',
    },
    {
      question: 'Quelle est la principale différence entre ResizeObserver et MutationObserver ?',
      sub: 'Choisir le bon observateur.',
      options: [
        'ResizeObserver observe les changements de taille d\'un élément, MutationObserver observe les modifications du DOM (attributs, nœuds)',
        'MutationObserver est plus récent et remplace ResizeObserver',
        'ResizeObserver surveille toute la page, MutationObserver surveille un seul élément',
        'Ils font la même chose mais avec une API différente',
      ],
      correct: 0,
      explanation: '✅ Exact ! ResizeObserver réagit exclusivement aux changements de dimensions (width, height) d\'un élément — peu importe la cause (redimensionnement de fenêtre, texte ajouté, etc.). MutationObserver réagit aux modifications structurelles du DOM : ajout/suppression de nœuds, changements d\'attributs, modification de texte. Chaque observateur a un domaine précis : utilise le bon outil selon ce que tu surveilles.',
    },
    {
      question: 'À quoi sert performance.mark() ?',
      sub: 'Performance API et profiling.',
      options: [
        'À marquer une ligne de code comme critique pour l\'optimiseur V8',
        'À nommer un point dans le temps pour mesurer des durées entre deux marks avec performance.measure()',
        'À activer le mode performance haute résolution du navigateur',
        'À enregistrer une métrique Core Web Vital manuellement',
      ],
      correct: 1,
      explanation: '✅ Exact ! performance.mark(\'nom\') pose un marqueur horodaté dans la timeline de performance. Tu places un mark au début et un à la fin d\'une opération, puis performance.measure(\'durée\', \'début\', \'fin\') calcule l\'intervalle. Ces mesures apparaissent dans l\'onglet Performance des DevTools Chrome, ce qui facilite le profiling visuel. C\'est bien plus précis que Date.now() car performance utilise une horloge monotone sub-milliseconde.',
    },
    {
      question: 'Quel est l\'objectif de requestIdleCallback() ?',
      sub: 'Planification de tâches non urgentes.',
      options: [
        'Exécuter du code en dehors du fil principal dans un Web Worker',
        'Différer l\'exécution d\'une tâche non urgente aux moments où le navigateur est inactif entre deux frames',
        'Annuler une opération asynchrone si le navigateur est trop occupé',
        'Remplacer requestAnimationFrame pour les animations haute fréquence',
      ],
      correct: 1,
      explanation: '✅ Exact ! requestIdleCallback() planifie une fonction pour qu\'elle s\'exécute durant les "creux" du cycle d\'événements, quand le navigateur n\'a rien d\'urgent à faire entre deux frames. C\'est idéal pour les tâches non critiques comme la pré-analyse de données, l\'envoi d\'analytics, ou la mise en cache — des opérations qui ne doivent pas bloquer les animations ou les interactions utilisateur.',
    },
    {
      question: 'Quelle est la différence entre structuredClone() et JSON.parse(JSON.stringify()) pour copier un objet ?',
      sub: 'Clonage profond d\'objets.',
      options: [
        'Aucune différence, les deux produisent le même résultat dans tous les cas',
        'structuredClone() est plus lent mais supporte les références circulaires et les types comme Date, Map, Set',
        'JSON.parse(JSON.stringify()) supporte plus de types que structuredClone()',
        'structuredClone() ne fonctionne que dans les Web Workers',
      ],
      correct: 1,
      explanation: '✅ Exact ! JSON.parse(JSON.stringify()) a plusieurs limitations : il ne peut pas gérer les références circulaires (erreur), il convertit les Date en strings, il perd les valeurs undefined, Map, Set, et les fonctions. structuredClone() (natif depuis 2022) gère les références circulaires, préserve les Date, Map, Set, ArrayBuffer, et supporte bien plus de types. Il est aussi généralement plus rapide pour les objets complexes.',
    },
  ],
};
